...
for iquantile = 1:4
    for icomponent = 1:length(component_name)
        ...
        for ibootstrap =1:bootstrap_num
            ...
            control_total_stock_ref = load([data_dir_output, 'world_simulation_analyses/compontent_control_soc_total_stock_', model_name, '_', nn_exp_name, ...
                '_bootstrap_', num2str(ibootstrap), '_control_test.mat']);

            control_total_stock = control_total_stock.var_data_middle;
            control_total_stock_ref = control_total_stock_ref.var_data_middle;
            % record
            control_matric_total_stock(ibootstrap) = control_total_stock(iquantile, icomponent);
            if icomponent == 2
                control_test = load([data_dir_output, 'world_simulation_analyses/compontent_control_spatial_variation_GCB_', model_name, '_', time_domain,...
            ...
            control_test_ref = load([data_dir_output, 'world_simulation_analyses/compontent_control_spatial_variation_', model_name, '_', time_domain,...
                '_', nn_exp_name, '_bootstrap_', num2str(ibootstrap), '.mat']);

            control_test = control_test.var_data_middle; % [iquantile, icomponent, imatric]
            control_test_ref = control_test_ref.var_data_middle;

            if icomponent == 1
                control_matric_stat(ibootstrap) = control_test(icomponent, stat_matric_index);
            else
                control_matric_stat(ibootstrap) = control_test(icomponent, stat_matric_index) - control_test_ref(1, stat_matric_index);
            end
        end
        % summary
        control_summary(counter, 1) = median(control_matric_total_stock, 'omitnan');
        control_summary(counter, 2) = quantile(control_matric_total_stock, 0.86);
        control_summary(counter, 3) = quantile(control_matric_total_stock, 0.14);
        control_summary(counter, 4) = median(control_matric_stat, 'omitnan');
        control_summary(counter, 5) = quantile(control_matric_stat, 0.86);
        control_summary(counter, 6) = quantile(control_matric_stat, 0.14);
...
counter = 1;
for iprocess = 1% 1:length(process_list)
    ...
    for ibootstrap = 1:bootstrap_num
        ...
        global_simu = load([data_dir_output, 'world_simulation_analyses/marginal_sensitivity_proda_GCB_', ...
...
save([data_dir_output, 'world_simulation_analyses/uncertainty_process_bootstrap_GCB_', model_name, '.mat'], 'uncertainty_process');
save([data_dir_output, 'world_simulation_analyses/marginal_change_summary_bootstrap_GCB_', model_name, '.mat'], 'marginal_change_total');
