...
model_name = 'cesm2_clm5_cen_vr_v2'; % 'cesm2_clm5_cen_vr_sce' % 'cesm2_clm5_cen_vr_v2';
...
for ibootstrap = start_id:end_id % cross_valid_id
...
%     cd(['/GFPS8p/cess11/taof/ensemble/src_da/src_', model_name, '/']);
    ...
    soc_stock_30cm = nan(ValidGrid, 3);
    soc_stock_100cm = nan(ValidGrid, 3);
    soc_stock_200cm = nan(ValidGrid, 3);
    soc_stock_total = nan(ValidGrid, 3);
    ...
    for iGrid = 1:length(GridInfo)
        ...
        warning off
        disp(['Processing cross valid ', num2str(ibootstrap), ' Grid: ', num2str(iGrid)]);
        ...
        soc_stock_30cm(iGrid, var_loc) = sum(soc_layer(1:4).*dz(1:4)) + soc_layer(5)*dz(5)*(0.3 - zisoi(4))/(zisoi(5) - zisoi(4));
        soc_stock_100cm(iGrid, var_loc) = sum(soc_layer(1:8).*dz(1:8)) + soc_layer(9)*dz(9)*(1 - zisoi(8))/(zisoi(9) - zisoi(8));
        soc_stock_200cm(iGrid, var_loc) = sum(soc_layer(1:11).*dz(1:11)) + soc_layer(12)*dz(12)*(2 - zisoi(11))/(zisoi(12) - zisoi(11));
        soc_stock_total(iGrid, var_loc) = sum(soc_layer(1:20).*dz(1:20));
    ...
    save_pathway = {[data_path, 'output_data/converged_soc/soc_simu_grid_info_',  model_name, '_', nn_exp_name, '.mat'], ...
					[data_path, 'output_data/converged_soc/soc_simu_30cm_',  model_name, '_', nn_exp_name, '.mat'], ...
					[data_path, 'output_data/converged_soc/soc_simu_100cm_',  model_name, '_', nn_exp_name, '.mat'], ...
					[data_path, 'output_data/converged_soc/soc_simu_200cm_',  model_name, '_', nn_exp_name, '.mat'], ...
					[data_path, 'output_data/converged_soc/soc_simu_total_',  model_name, '_', nn_exp_name, '.mat']
                    };
                
    save_var_namelist = {'grid_info', 'soc_stock_30cm', 'soc_stock_100cm', 'soc_stock_200cm', 'soc_stock_total'};
    
    save_var = {GridInfo, soc_stock_30cm, soc_stock_100cm, soc_stock_200cm, soc_stock_total};
